...
- **Breaking:** `dump` now emits MCP Description `0.8.0-rc.1` with its exact
...
- Replaced the legacy MCP SDK integration with `@modelcontextprotocol/client`
  2.0.0 and adopted `@mcpdesc/validator` 0.5.0 for exact RC.1 validation.
- Upgraded `@mcpdesc/core` to 0.4.0 and delegated RC.1 declaration selection
  and MCP Description 0.7.0 migration to shared semantic operations.
- Validation now dispatches MCP Description 0.8.0 documents by exact snapshot
  without falling back to a mutable latest schema; historical 0.7.0
...
- The deprecated `convert` command now validates and migrates mcpdesc 0.7.0
  documents to MCP Description 0.8.0 RC.1.
