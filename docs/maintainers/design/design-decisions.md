...
- **Standalone MCP Description spec repository (superseded).** This was initially rejected as too heavyweight, so v0.7 lived under [`spec/`](../../../spec/). That decision was later reversed: v0.8+ is developed and governed in the canonical [`mcpdesc/mcpdesc-specification`](https://github.com/mcpdesc/mcpdesc-specification) repository and published at [mcpdesc.org](https://mcpdesc.org/format/). The local specification and schemas are retained as v0.7-era history and compatibility assets.
